...
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
    ...
    \foreach \source in {1,...,\preInputNeurons}
      \foreach \dest in {1,...,\preHiddenNeurons}
        \path (preI-\source) edge (preH-\dest);
    ...
    \tikzstyle{every pin edge}=[->,shorten <=1pt, very thick, draw = black!80]
      ...
      \foreach \source in {1,...,\OutInputNeurons}
        \foreach \dest in {1,...,\OutHiddenNeurons}
          \path (postI-\source) edge (postH-\dest);
    ...
    \begin{scope}[on background layer]
      \node (pre)
        [fill = black!10, draw = black!60, thick,
         fit = {($(preI-1.left) + (-0.7,0)$) (preH-1) (preH-4) ($(preO-1.right) + (0.2,0)$)}] {};
      \node (reservoir)
      ...
      \node (post)
        [fill = black!10, draw = black!60, thick,
         fit = {($(postI-1.left) + (-0.9,0)$) (postH-1) (postH-5) ($(postO-1.right) + (0.1,0)$)}] {};
    \end{scope}

    \node[annot, above of=preH-1, node distance = 1cm]  {Input processing};
    \node[annot, above of=Ra, node distance = 1.4cm]  {Reservoir};
    \node[annot, above of=postH-1, node distance = 0.9cm] {Output layer};
\end{tikzpicture}
